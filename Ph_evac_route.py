...
def _overpass_query(query, label=""):
    """
    Try each Overpass mirror in turn, with one retry on timeout.
    Returns parsed JSON elements list, or raises on total failure.
    """
    last_exc = None
    for mirror in OVERPASS_MIRRORS:
        for attempt in (1, 2):
            try:
                if attempt == 2:
                    print(f"  ⏳  Retrying on {mirror.split('/')[2]}...")
                    time.sleep(3)
                ...
                last_exc = f"Timeout on {mirror.split('/')[2]}"
                print(f"  ⏰  {last_exc}, trying next mirror...")
                break   # move to next mirror immediately on timeout
            except Exception as exc:
                last_exc = str(exc)
                if attempt == 1:
                    continue
...
def fetch_osm_data(bbox):
    """
    Returns (ways, shelters).

    ways: list of {id, tags, geom}
      geom = [(lat, lon), ...] — the REAL ordered coordinate sequence for
      this way, exactly as stored in OSM. No interpolation, no snapping.

    shelters: list of {lat, lon, name, capacity, stype}
    """
    S, W, N, E = bbox
...
(
  ...
  node["amenity"~"school|university|community_centre|community_center|townhall|hospital|clinic|social_facility|place_of_worship|gym|gymnasium|public_building|barangay_hall|civic_centre"]({S},{W},{N},{E});
  way["amenity"~"school|university|community_centre|community_center|townhall|hospital|clinic|social_facility|place_of_worship|gym|gymnasium|public_building|barangay_hall|civic_centre"]({S},{W},{N},{E});
  node["building"~"civic|government|school|university|church|chapel|barangay_hall|gymnasium|sports_hall|public"]({S},{W},{N},{E});
  way["building"~"civic|government|school|university|church|chapel|barangay_hall|gymnasium|sports_hall|public"]({S},{W},{N},{E});
  node["emergency"~"evacuation_centre|assembly_point|evacuation_center"]({S},{W},{N},{E});
  way["emergency"~"evacuation_centre|assembly_point|evacuation_center"]({S},{W},{N},{E});
  node["leisure"~"sports_centre|stadium|sports_hall|fitness_centre"]({S},{W},{N},{E});
  way["leisure"~"sports_centre|stadium|sports_hall|fitness_centre"]({S},{W},{N},{E});
  node["religion"~"christian|catholic"]({S},{W},{N},{E});
  node["office"~"government|administrative"]({S},{W},{N},{E});
  way["office"~"government|administrative"]({S},{W},{N},{E});
);
out geom;
"""
    print(f"\n  🌐  Fetching OSM data ({S:.4f},{W:.4f})–({N:.4f},{E:.4f})...")
    print(f"      Trying Overpass API (may take up to 60 sec for dense areas)...")
    try:
    ...
    for el in elements:
        ...
        if etype == "way" and "highway" in tags and "geometry" in el:
            ...
            if len(geom) >= 2:
                ways.append({"id": el["id"], "tags": tags, "geom": geom})

        # ── Shelter nodes ──────────────────────────────────────────────────────
        elif etype == "node" and _is_shelter(tags):
            shelters.append({
                "lat": el["lat"], "lon": el["lon"],
                "name": _shelter_name(tags, el["lat"], el["lon"]),
                "capacity": _shelter_capacity(tags),
                "stype": _shelter_type(tags),
            })

        # ── Shelter ways (centroid) ────────────────────────────────────────────
        elif etype == "way" and _is_shelter(tags) and "geometry" in el:
    ...
    # If no shelters found at all, add a synthetic fallback at the bbox center
    # so the router always has something to route toward
    if not shelters:
        clat = (S + N) / 2
        clon = (W + E) / 2
        print("  ⚠  No tagged shelters found — adding bbox center as fallback point.")
        print("     (This area may lack OSM shelter tags. Verify with local DRRMO.)")
        shelters = [{
            "lat": clat, "lon": clon,
...
def build_graph(start_lat, start_lon, ways, shelters,
    ...
    """
    node_info = {}   # node_key -> (lat, lon, label, ntype, capacity)

    def reg(lat, lon, label="", ntype="road", cap=0):
        key = (round(lat,6), round(lon,6))
        if key not in node_info:
            node_info[key] = (lat, lon, label, ntype, cap)
        return key
    ...
    shelter_ids = set()
    for sh in shelters:
        sid = reg(sh["lat"], sh["lon"], sh["name"], "shelter", sh["capacity"])
        shelter_ids.add(sid)
...
def draw_map(start, path, segs, node_info, raw_edges,
    ...
    drrmo_lgu, drrmo_number = drrmo_info
    ...
    for nid, nd in node_info.items():
        ...
        icon_html = (
            f'<div style="background:{"#00cc44" if is_best else "#66bb33"};'
            ...
            f'box-shadow:{"0 0 10px #00ff88" if is_best else "none"};">🏥</div>'
        )
    ...
    if path:
        ...
        steps_html = "".join(
            f'<div style="margin:3px 0"><b>{i+1}.</b> {s["road"]} '
            f'<span style="color:{ROAD_COLOR[s["flood"]]};font-weight:bold;">[{s["flood"]}]</span>'
            f'<br>&nbsp;&nbsp;{s["dist_m"]}m</div>'
            for i,s in enumerate(segs))
        init_html = f"""
          <div id="panel-title" style="color:#00ff88;font-size:15px;font-weight:bold;margin-bottom:8px;">⭐ RECOMMENDED ROUTE</div>
          <div>📍 <b id="panel-from">{snd[2][:50]}</b></div>
          <div>🏥 → <b id="panel-to">{dstn[2][:50]}</b></div>
          <div>👥 Capacity: <span id="panel-cap">{dstn[4]}</span> persons</div>
          <div>📏 <span id="panel-dist">{td}</span>m &nbsp;|&nbsp; ⏱ ~<span id="panel-time">{tm:.0f}</span> min walk</div>
          <div style="font-size:10px;color:#88aacc;margin-top:4px;">W = {alpha}D + {beta}R + {gamma}C + {delta}P &nbsp;({opt_name.strip()})</div>
          <div id="panel-steps" style="margin-top:8px;border-top:1px solid #006622;padding-top:7px;">{steps_html}</div>
          <div style="margin-top:7px;font-size:10px;color:#556;border-top:1px solid #1a3a5c;padding-top:5px;">✅ Dry &nbsp;🟢 Low &nbsp;🟠 Moderate &nbsp;🔴 High &nbsp;⛔ Blocked</div>
          <div style="margin-top:8px;font-size:11px;color:#ff9900;border-top:1px solid #1a3a5c;padding-top:6px;">🚶 <b>Drag the orange person</b> on the map to re-route!</div>"""
        panel_border = "#00cc55"
    else:
        init_html = """
          <div style="color:#ff3300;font-size:16px;font-weight:bold;margin-bottom:8px;">⛔ NO SAFE ROUTE</div>
          <div>All paths to shelters are flooded or blocked.</div>
          <div style="margin-top:6px;">➤ Move to nearest high ground.</div>
          <div style="color:#ff9900;font-weight:bold;margin-top:5px;">📞 NDRRMC: 8-911</div>
          <div style="margin-top:8px;font-size:11px;color:#ff9900;">🚶 <b>Drag the orange person</b> to try a different spot!</div>"""
        panel_border = "#ff0000"

    m.get_root().html.add_child(folium.Element(f"""
    <div id="route-panel" style="position:fixed;bottom:20px;left:20px;z-index:9999;
        background:rgba(8,15,26,0.95);border:2px solid {panel_border};border-radius:10px;
        padding:14px 16px;font-family:monospace;color:white;
        max-width:320px;font-size:12px;line-height:1.5;max-height:72vh;overflow-y:auto;">
      {init_html}
    </div>"""))
    ...
    nodes_js = {}
    for nid, nd in node_info.items():
        nodes_js[str(nid)] = [round(nd[0],6), round(nd[1],6), nd[2][:40], nd[3], nd[4]]
...
(function() {
  ...
  var PERSON_SVG =
    '<div style="width:40px;height:52px;cursor:grab;filter:drop-shadow(0 3px 5px rgba(0,0,0,0.6));">'+
    '<svg viewBox="0 0 40 52" xmlns="http://www.w3.org/2000/svg">'+
      '<ellipse cx="20" cy="14" rx="14" ry="14" fill="#FF6B00"/>'+
      '<path d="M6 14 Q6 34 20 50 Q34 34 34 14" fill="#FF6B00"/>'+
      '<circle cx="20" cy="9" r="4" fill="white"/>'+
      '<line x1="20" y1="13" x2="20" y2="23" stroke="white" stroke-width="2.5"/>'+
      '<line x1="20" y1="17" x2="13" y2="21" stroke="white" stroke-width="2.5"/>'+
      '<line x1="20" y1="17" x2="27" y2="21" stroke="white" stroke-width="2.5"/>'+
      '<line x1="20" y1="23" x2="15" y2="30" stroke="white" stroke-width="2.5"/>'+
      '<line x1="20" y1="23" x2="25" y2="30" stroke="white" stroke-width="2.5"/>'+
    '</svg></div>';

  var personIcon = L.divIcon({
    html: PERSON_SVG,
    iconSize: [40,52], iconAnchor: [20,50], className: ''
  });

  // Poll for the Leaflet map instance
  var attempts = 0;
  var poll = setInterval(function() {
    attempts++;
    if (attempts > 50) { clearInterval(poll); return; }

    // Find the Folium map object — it's the first L.Map instance on the page
    var mapObj = null;
    for (var key in window) {
      try {
        var v = window[key];
        if (v && typeof v === 'object' && v._leaflet_id !== undefined &&
            typeof v.addLayer === 'function' && typeof v.getCenter === 'function') {
          mapObj = v; break;
        }
      } catch(e) {}
    }
    ...
    // ── DEMO MODE — auto re-route from random nodes on a loop ─────────────
    var demoInterval = null;
    var demoRunning  = false;
    var demoCount    = 0;

    // Build demo node pool: only nodes where Dijkstra can actually find a shelter.
    // We test a sample of road nodes upfront so demo never gets stuck.
    var _allRoadNodes = Object.keys(GD.nodes).filter(function(id){
      var nd = GD.nodes[id];
      return nd && nd[3] !== 'shelter' && nd[3] !== 'start';
    });
    ...
    // Test up to 120 random nodes — keep those that can reach a shelter
    var _testPool = _shuffle(_allRoadNodes.slice()).slice(0, 120);
    var _roadIds  = [];
    for (var _ti=0; _ti<_testPool.length; _ti++) {
      var _tid = _testPool[_ti];
      if (adj[_tid] && dijkstra(_tid) !== null) {
        _roadIds.push(_tid);
      }
    }
    // If too few passed, fall back to all road nodes (demo will just skip failures)
    if (_roadIds.length < 5) { _roadIds = _allRoadNodes; }

    function randomRoadNode() {
      if (_roadIds.length === 0) return _allRoadNodes[0] || Object.keys(GD.nodes)[0];
      return _roadIds[Math.floor(Math.random() * _roadIds.length)];
    }

    var demoIntervalSec = 3;

    function demoStep() {
      if (!demoRunning) return;
      demoCount++;
      var nid = randomRoadNode();
      var nd  = GD.nodes[nid];
      if (!nd) { scheduleNextDemo(); return; }

      person.setLatLng([nd[0], nd[1]]);
      mapObj.panTo([nd[0], nd[1]], { animate: true, duration: 0.5 });

      var panel = document.getElementById('route-panel');
      if (panel) {
        panel.style.borderColor = '#0088ff';
        panel.innerHTML = '<div style="color:#0088ff;font-size:13px;">🔄 Demo run #' + demoCount + '...</div>';
      }

      clearRoutes(mapObj);
      var result = dijkstra(nid);
      if (result && result.segs.length > 0) {
        // Draw route, then wait for animation to finish before scheduling next
        drawRoutes(mapObj, result.segs);
        updatePanel(result.segs, result.shelter, nd[2] || 'Random location');
        // Estimate animation duration: ~20ms per segment fade + 60ms gap
        var animMs = result.segs.length * 80 + 400;
        var waitMs = Math.max(demoIntervalSec * 1000, animMs + 300);
        demoInterval = setTimeout(demoStep, waitMs);
      } else {
        noRoutePanel();
        scheduleNextDemo();
      }
    ...
    function scheduleNextDemo() {
      if (!demoRunning) return;
      demoInterval = setTimeout(demoStep, demoIntervalSec * 1000);
    }

    function startDemo(intervalSec) {
      if (demoRunning) return;
      demoRunning     = true;
      demoCount       = 0;
      demoIntervalSec = intervalSec;
      demoStep();  // run immediately
      document.getElementById('demo-btn').textContent = '⏹ Stop Demo';
      document.getElementById('demo-btn').style.background = '#cc2200';
    }
    ...
    function stopDemo() {
      ...
      document.getElementById('demo-btn').style.background = '#005522';
    }

    // Expose to window so onclick="toggleDemo()" works from outside the closure
    window.toggleDemo = function() {
      if (demoRunning) { stopDemo(); } else {
        var sec = parseFloat(document.getElementById('demo-speed').value) || 3;
        startDemo(sec);
      }
    };

    // Inject demo controls into the page
    var demoDiv = document.createElement('div');
    demoDiv.style.cssText = [
      'position:fixed','bottom:52px','right:18px','z-index:99999',
      'background:rgba(10,20,10,0.95)','border:2px solid #00cc55',
      'border-radius:10px','padding:10px 14px','display:flex',
      'align-items:center','gap:10px','box-shadow:0 4px 16px rgba(0,200,80,0.3)',
      'pointer-events:all'
    ].join(';');
    demoDiv.style.setProperty('pointer-events','all','important');
    demoDiv.innerHTML = [
      '<span style="color:#aaffcc;font-size:12px;white-space:nowrap">⏱ Interval (s):</span>',
      '<input id="demo-speed" type="number" min="1" max="60" value="3" ',
        'style="width:48px;background:#0a1a0a;border:1px solid #335;color:#eee;',
        'border-radius:5px;padding:3px 6px;font-size:12px;">',
      '<button id="demo-btn" onclick="toggleDemo()" ',
        'style="background:#005522;border:1px solid #00cc55;color:#aaffcc;',
        'border-radius:6px;padding:6px 14px;cursor:pointer;font-size:12px;',
        'font-weight:bold;white-space:nowrap">▶ Demo Mode</button>'
    ].join('');
    // Append outside Leaflet so Leaflet's pointer-events:none doesn't block it
    document.body.appendChild(demoDiv);
    // Force pointer events on button specifically
    var btn = document.getElementById('demo-btn');
    if (btn) btn.style.setProperty('pointer-events','all','important');
    var spd = document.getElementById('demo-speed');
    if (spd) spd.style.setProperty('pointer-events','all','important');
